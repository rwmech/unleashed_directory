...
- Filters the board list by badge: a Filter button opens every badge as small chips, a row per group, and picking some shows the boards carrying all of them, or any. It works without JavaScript, and every filtered view is a link, such as `/directory?b=petscii&b=ham`. The front page shows the ten busiest boards; `/directory` has every board, the filter and a search by name, sysop or description (`?q=`).
