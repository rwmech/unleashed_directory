...
#              changed. Pulls the guides too, when a checkout of
#              unleashed_documentation is named in
#              /etc/unleashed-directory/docs. Safe to run when there is
#              nothing to do.
#
...
#                                                      exit
#
# It reuses the domain the install was given, remembered in
# /etc/unleashed-directory/domains, so an update never has to be told it
# again and cannot get it wrong.
#
# Since the split (2026-09-26) the browser installer and its firmware are
# the main site's (unleashed_site), which has its own update.sh; this one
# no longer fetches firmware releases.
#
...
DOCS_FILE=/etc/unleashed-directory/docs
# setup.sh writes the commits it installed here once it has finished. The
# variables are for testing this script away from a real droplet.
INSTALLED_FILE="${DIRECTORY_INSTALLED_FILE:-/srv/unleashed_directory/.installed}"
INSTALLED_DOCS_FILE="${DIRECTORY_INSTALLED_DOCS_FILE:-/srv/unleashed_directory/.installed-docs}"
QUIET=0
CHECK=0
...
# The guides: a checkout named in DOCS_FILE, pulled the same way. None named
# is fine: there is then no /docs, and nothing here mentions guides again.
DOCS="$(cat "$DOCS_FILE" 2>/dev/null || true)"
DOCS_OLD=""
DOCS_NEW=""
if [ -n "$DOCS" ]; then
    if [ -d "$DOCS/.git" ]; then
        DOCS_OLD="$(git -C "$DOCS" rev-parse HEAD)"
        git -C "$DOCS" fetch --quiet origin
        DOCS_NEW="$(git -C "$DOCS" rev-parse '@{u}')"
    else
        loud "$DOCS_FILE names $DOCS, which is not a git checkout; the guides are not updated."
        DOCS=""
    fi
fi

# The commits the last finished install put live, if any (site 1.2.1). A
# pull that moved HEAD and then an install that failed leaves the checkout
# newer than the site, and without this the next run would find nothing to
# pull and never install it.
INSTALLED="$(cat "$INSTALLED_FILE" 2>/dev/null || true)"
INSTALLED_DOCS="$(cat "$INSTALLED_DOCS_FILE" 2>/dev/null || true)"

moved=0
[ "$OLD" != "$NEW" ] && moved=1
[ -n "$DOCS" ] && [ "$DOCS_OLD" != "$DOCS_NEW" ] && moved=1
stale=0
[ "$INSTALLED" != "$OLD" ] && stale=1
[ -n "$DOCS" ] && [ "$INSTALLED_DOCS" != "$DOCS_OLD" ] && stale=1

if [ "$moved" -eq 0 ] && [ "$stale" -eq 0 ]; then
    say "Already up to date at $(git log -1 --format='%h %s')"
    [ -n "$DOCS" ] && say "Guides at $(git -C "$DOCS" log -1 --format='%h %s')"
    exit 0
fi
...
if [ "$CHECK" -eq 1 ]; then
    if [ "$moved" -eq 1 ]; then
        ...
        [ -n "$DOCS" ] && git -C "$DOCS" log --oneline "$DOCS_OLD..$DOCS_NEW" | sed 's/^/  guides: /'
    else
        loud "The checkout was never installed: $(git log -1 --format='%h %s')"
    fi
    exit 2
fi
...
# --ff-only rather than a merge: if a checkout has been edited on the box,
...
if [ "$OLD" != "$NEW" ]; then
    ...
    # What changed, in words rather than commit subjects.
...
if [ -n "$DOCS" ] && [ "$DOCS_OLD" != "$DOCS_NEW" ]; then
    if ! git -C "$DOCS" merge --ff-only "$DOCS_NEW" >/dev/null 2>&1; then
        loud "Cannot update the guides: $DOCS has local changes or has diverged."
        exit 1
    fi
    loud "Guides updated:"
    git -C "$DOCS" log --oneline "$DOCS_OLD..$DOCS_NEW" | sed 's/^/  /'
fi

# ---------------------------------------------------------------------------
# Re-run the installer with the domain it was given the first time, so one
...
if [ "$QUIET" -eq 1 ]; then
    DOCS_SRC="$DOCS" "$SRC/deploy/setup.sh" $DOMAINS > /tmp/unleashed-update.log 2>&1 || {
        loud "The install failed. Last lines:"
        tail -20 /tmp/unleashed-update.log | sed 's/^/  /'
        exit 1
    }
else
    DOCS_SRC="$DOCS" "$SRC/deploy/setup.sh" $DOMAINS
fi
...
# The badges page lives in pages/, which is a separate thing to deploy and
# so a separate thing to forget. The build page was forgotten once already.
check "the badges page renders"    "curl -fsS --max-time 10 http://127.0.0.1:8080/badges"
if [ -n "$DOCS" ]; then
    check "the guides render"      "curl -fsS --max-time 10 http://127.0.0.1:8080/docs/setup"
fi
...
if [ -s "$NOTIFY_FILE" ]; then
    ...
    curl -fsS --max-time 15 -X POST "$url" -H 'Content-Type: text/plain' \
         -d "$(hostname): directory $state
