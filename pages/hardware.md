...
The ESP32-S3 boards carry a second ribbon, with a padlock: **Secure
communications**, marked **coming**. They are the boards getting [[SSH]], an
encrypted way to connect, beside telnet. It is planned and not built yet, and
the ribbon will say so the day a release carries it.
...
- **Encrypted connections, coming.** [[SSH]] beside telnet, so what a caller
  types is scrambled on the way. Planned for the S3 boards only and not built
  yet; telnet stays, for the machines that cannot do it. [On the
  roadmap](/roadmap).
...
beside telnet, **coming** and not built yet, the same as the Waveshare.
