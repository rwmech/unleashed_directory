...
All the domains can point at the same address. The setup script takes several and sends the extras to the first one, which is what you want when you bought the `.net` and the `.org` out of caution.
...
**Boards report "refused" but the website works.** Something is redirecting `/announce` to HTTPS. Boards cannot follow it. Check that the `http://` block in `/etc/caddy/Caddyfile` still handles `/announce` before its redirect.
